...
qs("join").onclick=()=>{
 ...
 if(code){room=code; socket.emit("join_room",{room:code});}
};
...
socket.on("state",data=>{state=data;updateUI();drawBoard();});
...
function drawBoard(){
 ...
 for(let r=0;r<state.size;r++){
   for(let c=0;c<state.size;c++){
     ...
     if(state.phase==="placing_p1" && state.minesP1.includes(pos)) cell.textContent="●";
     if(state.phase==="placing_p2" && state.minesP2.includes(pos)) cell.textContent="●";

     if(state.phase==="playing" && state.dug.includes(pos)){
        cell.classList.add("dug");
        if(state.minesP1.includes(pos)||state.minesP2.includes(pos)) cell.textContent="💥";
     }
