...
function drawBoard(){
  ...
  for(let r=0;r<state.size;r++){
    for(let c=0;c<state.size;c++){
      ...
      if(state.phase==="placing_p1" && state.mines1.includes(pos)){
        cell.textContent="●";
      }
      if(state.phase==="placing_p2" && state.mines2.includes(pos)){
        cell.textContent="●";
      }

      if(state.phase==="playing" && state.dug.includes(pos)){
        cell.classList.add("dug");
        if(state.mines1.includes(pos)||state.mines2.includes(pos))
          cell.textContent="💥";
      }
